...
function dfa_gen_impl(::Val{tbl}, ::Val{start}, xs) where {tbl,start}
    ...
    for (s, tbl_s) in tbl
        push!(body, :(@label $s))
        push!(body, :(xs_it !== nothing || return ys))
        push!(body, :((x, xs_state) = xs_it))
        push!(body, :(xs_it = iterate(xs, xs_state)))
        for (x0, (ns, r)) in tbl_s
            push!(body, :(x == $x0 && (ys = (ys..., $(Meta.quot(r))); @goto $ns)))
        end
...
```{julia}
#| output: false
@generated function dfa_gen(::Val{tbl}, ::Val{state}, xs) where {tbl,state}
    dfa_gen_impl(Val(tbl), Val(state), xs)
end
```
...
@ct_enable function dfa_gen_ct(@ct(tbl), @ct(start), xs)
    ...
    @ct_ctrl for (s, tbl_s) in tbl
        ...
        (x, xs_state) = xs_it
        xs_it = iterate(xs, xs_state)
        @ct_ctrl for (x0, (ns, r)) in tbl_s
            x == @ct(x0) && (ys = (ys..., @ct(r)); @goto @ct(ns))
        end
        error("Unexpected input: ", x)
    end
end
