...
Подключаем функцию `cleanup`, для подчистки сгенерированных программ,
...
#align(center)[
  #diagram(
    ...
    spacing: (5em, 2.2em),
    {
      let ed(n1, n2, label, ..others) = {
        edge(n1, n2, "-|>", label, label-side: center, ..others)
      }

      node((-1, 0), `s0`, name: <s0>)
      node((+1, 0), `s1`, name: <s1>)
      ed(<s0.north>, <s0.west>, [`0/even`], bend: -152deg)
      ed(<s0.north>, <s1.north>, [`1/odd`], bend: 30deg)
      ed(<s1.south>, <s1.east>, [`0/odd`], bend: -152deg)
      ed(<s1.south>, <s0.south>, [`1/even`], bend: 30deg)
    },
  )
]
